...
public class ReceiverFSM {
    ...
    public int eventaction(String event, receiverSite r, int seqNo,int dseqNo, DatagramSocket socket, dataPacket packet , InetAddress addr , int portNumber)
    {
        if(event.equals("packetReceived"))return currentState.action(event, r, seqNo, dseqNo, socket, packet , addr , portNumber);
        return 2;

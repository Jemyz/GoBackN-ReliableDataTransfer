...
public class wfbstate extends receiverState {
    ...
    @Override
    public int action(String event, receiverSite r, int seqNo, int dseqNo, DatagramSocket socket, dataPacket packet, InetAddress addr, int portNumber) {

        int senderSeqNo = dseqNo;
        ...
        if (seqNo == senderSeqNo && r.validateCKSum(packet)) {
            ...
            try {
                socket.send(r.getDatagramPacket(r.makeACKPacket(seqNo-1), addr, portNumber));
